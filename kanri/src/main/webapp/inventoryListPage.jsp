...
<body>
	...
	<form style="display: flex;" action="SearchByElseHandler" method="post">
		...
		<div>
			<a>금액:</a> <input type="range" id="search_Amount"
				name="search_Amount" min="0" max="100000" step="10000" value="0"
	...
	<table>
		...
		<c:if test="${articlePage.hasArticles() }">
			<tr>
				<td colspan="4"><c:if test="${articlePage.start_Page>10 }">
						<a href="list.do?pageNo=${articlePage.start_Page -10 }">[이전]</a>
					</c:if> <c:forEach var="pNo" begin="${articlePage.start_Page }"
						end="${articlePage.end_Page }">
						<a href="list.do?pagNo=${pNo }">[${pNo}]</a>
					</c:forEach> <c:if test="${articlePage.end_Page < articlePage.total_Pages}">
						<a href="list.do?pageNo=${articlePage.start_Page +10 }">[다음]</a>
					</c:if></td>
			</tr>
		</c:if>
